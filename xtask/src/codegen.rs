...
fn emit_profile_module(p: &ProfileData) -> String {
    ...
    // The `ahb_rule_pack` / `mig_rule_pack` dispatch is a generated match with one
    // arm per Prüfidentifikator; large profiles (e.g. UTILMD Strom) exceed the
    // default 100-line threshold.
