...
```toml
[dependencies]
edi-energy  = { version = "0.13", features = ["utilmd", "mscons"] }
mako-engine = { version = "0.13", features = ["slatedb"] }
mako-gpke   = "0.13"   # or mako-wim / mako-geli-gas / mako-mabis
```
...
```toml
[dependencies]
energy-api = { version = "0.13", features = ["client"] }   # HTTP client
energy-api = { version = "0.13", features = ["server"] }   # Axum server handler
```
