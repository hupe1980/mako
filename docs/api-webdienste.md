...
```toml
[dependencies]
edi-energy  = { version = "0.10", features = ["utilmd", "mscons"] }
mako-engine = { version = "0.10", features = ["slatedb"] }
mako-gpke   = "0.10"   # or mako-wim / mako-geli-gas / mako-mabis
```
...
```toml
[dependencies]
energy-api = { version = "0.10", features = ["client"] }   # HTTP client
energy-api = { version = "0.10", features = ["server"] }   # Axum server handler
```
