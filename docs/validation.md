...
// Filtered by validation layer origin (F-026, F-021 partial)
...
```toml
edi-energy = { version = "0.1", features = ["serde"] }
```
...
```toml
edi-energy = { version = "0.1", features = ["diagnostics"] }
```
...
Rule identifiers follow a hierarchical naming pattern:

```
{layer}-{pid_or_type}-{segment}-{qualifier}{occurrence}

MIG-DTM-001          # MIG rule on DTM segment
AHB-55001-STS-I0     # AHB rule, PID 55001, STS segment, "info 0"
AHB-55001-DTM-M0     # AHB rule, PID 55001, DTM, mandatory occurrence 0
SEM-UTILMD-DATE-COH  # Semantic date-coherence rule
```

Use `issues_with_rule_prefix` to scope to a specific area:
