...
description = "Operator guide for netzbilanzd — the NNE/KA/MMM/MSB/AWH billing daemon that generates INVOIC 31001, 31002, 31005, 31009, and 31011 invoices for the NB (Netzbetreiber) role, self-validates via invoic-checker, dispatches via makod, and closes the payment lifecycle on REMADV receipt."
weight = 25
[extra]
mermaid = true
+++
...
### §42b EnWG GGV — Community solar / shared metering
...
| `ggv-nne-billing` | §42b EnWG GGV community solar multi-tenant NNE |
...
| §42b EnWG | GGV NNE: each tenant MaLo billed individually for proportional NNE share |
