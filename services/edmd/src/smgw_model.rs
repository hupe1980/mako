...
//! configuration are none of those things: they are device administration, and
//! they change without a single metered value changing.
//!
//! So the types came here, where they were already being persisted. `edmd` owns
//! the `smgw_sessions` table and the two compliance sweeps that read it, which
//! makes it the layer that actually has a stake in this model.
//!
//! **The serde representation is load-bearing and unchanged.** Sessions are
//! stored as JSONB, so every field name and every `SCREAMING_SNAKE_CASE`
//! rename here matches what `metering` 0.16 wrote — rows already in the
//! database deserialise into these types untouched.
//!
//! ## Legal basis
//!
