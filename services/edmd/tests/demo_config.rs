//! Guard: every `demos/` config file `edmd` loads is one `edmd` accepts.
//!
...
//! with `EDMD_*` environment variables, so a key the struct does not declare is
...
const DEMO_CONFIGS: &[&str] = &["demos/eeg-billing/edmd.toml"];
...
fn the_demo_configs_name_no_key_the_service_ignores() {
    for rel in DEMO_CONFIGS {
        ...
        if let Err(e) = toml::from_str::<edmd::config::Config>(&src) {
            let msg = e.to_string();
            assert!(
                !msg.contains("unknown field"),
                "{rel} names a key `edmd` does not have — fix the demo, or the config:\n{msg}"
            );
        }
    }
}
