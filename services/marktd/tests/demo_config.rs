//! Guard: every `demos/` config file `marktd` loads is one `marktd` accepts.
//!
...
//! with `MARKTD_*` environment variables, so a key the struct does not declare is
...
const DEMO_CONFIGS: &[&str] = &["demos/eeg-billing/marktd.toml", "demos/nb-stp/marktd.toml"];
...
fn the_demo_configs_name_no_key_the_service_ignores() {
    for rel in DEMO_CONFIGS {
        ...
        if let Err(e) = toml::from_str::<marktd::config::Config>(&src) {
            let msg = e.to_string();
            assert!(
                !msg.contains("unknown field"),
                "{rel} names a key `marktd` does not have — fix the demo, or the config:\n{msg}"
            );
        }
    }
}
