//! Guard: invoicd refuses to start in a posture that admits every caller.
//!
...
fn config(extra: serde_json::Value) -> Config {
    let mut cfg = serde_json::json!({"http": {}, "database": {"url": "postgres://localhost/invoicd"}, "identity": {"tenant": "9900357000004"}, "makod": {"url": "http://localhost:8080"}, "marktd": {"url": "http://localhost:8180", "api_key": "test"}});
...
fn startup_refuses_without_oidc() {
    ...
    for named in ["[oidc]", "§ 147 AO", "Zahlungsstatus", "REMADV/COMDIS"] {
        assert!(
            msg.contains(named),
...
fn startup_refuses_an_unsigned_inbound_webhook() {
    ...
    let msg = err.to_string();
    for named in ["webhook.inbound_secret", "unsigned"] {
        assert!(
            msg.contains(named),
