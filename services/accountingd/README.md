...
| **Auth** | OIDC/JWT **+ Cedar ABAC** on every endpoint, plus inbound webhook HMAC-SHA256. Reads split three ways — `read-account`, `read-banking`, `read-books` — and `/mcp` is held to the same actions, one per tool |
| **Auth guard** | `tests/authorization_guard.rs` fails the build on a lost `Claims` extractor, a lost Cedar check, or a mutating MCP tool mapped to a read |
| **Ledger** | The [`doubleentry`](https://github.com/hupe1980/doubleentry) crate: **immutable and tamper-evident** — balanced by construction, an append-only Merkle log with inclusion/consistency proofs, period seals (GoBD/§ 146 AO), open-item clearing, store-level idempotency |
| **Ledger boundary** | accountingd owns the chart of accounts (`ledger::Chart`) and the `entry_type → postings` mapping; every money movement flows through `pg::post_entry` → `ledger.post` |
...
| **Verzugszinsen** | `GET/POST /api/v1/accounts/{malo_id}/interest-charges` — § 288 BGB on the § 247 Basiszinssatz + 5 pp (B2C) / + 9 pp (B2B), booked to a **Zinsertrag** GL account under its own Buchungsart (§ 275 HGB reports them on their own line) |
| **Zins-Stichtage** | The Basiszinssatz moves on 1 January and 1 July, so a Verzugszeitraum is split at every Stichtag it crosses and each segment charged at its own rate. An unseeded period is refused, never estimated |
...
| **CORE vs B2B** | The scheme sits on the mandate **and** on every collection entry (a pain.007 restates the original as submitted); one `PmtInf` group carries exactly one scheme |
| **Why the scheme matters** | A CORE debtor has an unconditional 8-week refund right; a B2B debtor has none, and their bank must hold the mandate |
...
| **Bank statement import** | `POST /api/v1/payments/import/camt053` (end-of-day) · `.../camt054` (intraday) · `.../camt052` (intraday report) · `.../import` (flat JSON) — one pipeline, one sign convention, deduplicated on the bank's transaction reference |
| **Only `BOOK` posts** | `INFO` is not a money movement and `PDNG` may still be dropped — and an append-only ledger cannot un-book |
| **Payment resolution** | Strongest evidence first — counterparty IBAN → `EndToEndId` → an exact Mandatsreferenz/MaLo-ID token in the Verwendungszweck. Ambiguous references resolve to nothing and are counted `unresolved` |
| **Why that order** | IBAN alone loses every payment made from a spouse's or employer's account; whole-token matching (never substring) stops a stranger's payment landing on a customer |
...
| **camt.055 recall** | `POST /api/v1/sepa/recalls` — asks the bank to stop a collection **before** it settles, whole-message or by `EndToEndId`. A request, not an act: the entries stay `SUBMITTED` until `POST /api/v1/sepa/camt029` applies the bank's answer, matched on `RslvdCase/Id` |
| **IBAN lookup key** | `iban_hash` is an app-computed **keyed BLAKE3** key (no pgcrypto), and CAMT.054 matching runs on it rather than on the IBAN. The `iban_encrypted` column is declared and read by nothing — encryption at rest is the deployment's, not this service's |
...
| **Mahnung delivery** | Each open case is rendered and delivered as a `MAHNUNG` through `outputd` (recipient from `vertragd`); a case that cannot be addressed is not documented |
| **Sperr-Sequenz (§§41f/41g EnWG)** | Four phases under the same § 41f Abs. 3 gates: **Sperrandrohung** (Abs. 1, 4 Wochen) → **Sperrankündigung** (Abs. 5, 8 Werktage brieflich) → **Sperrauftrag** (ORDERS **17115** via `makod`) → **Entsperrauftrag** (Abs. 7, ORDERS **17117**) |
| **What both notices carry** | The Grund, the voraussichtlichen Unterbrechungs-/Wiederherstellungskosten (Abs. 6) and the avoidance options (Abs. 4) |
...
| **Forderungseinwände (§41f Abs. 3 S. 3–5)** | Amounts that stay out of the Verzug: a claim disputed form- und fristgerecht, a disputed price increase, a claim before a § 111b Schlichtung, instalments not yet due |
| **Einwände are not halts** | They reduce what the threshold is measured against, so the sequence stops by itself once what remains falls below it |
...
| **Metrics** | `GET /accountingd/metrics` — Prometheus gauges (open receivables, credit balances, dunning by Mahnstufe, pending SEPA runs). Its own path: the runner owns `/metrics`, and a service claiming it too panics `Router::merge` at startup |
| **Worker safety** | Abschlag/dunning workers hold a PostgreSQL advisory lock (`mako_service::worker_lock`, shared with `vertragd` — the key space is per service, `0x_acc0_xxxx` here); all money workers are idempotent (per-run guards) |
| **Jahresabschluss** | Annual settlement (§ 40 EnWG), idempotent per year via `jahresabschluss_runs`. On demand, or from the § 40b Abs. 1 worker (`jahresabschluss_auto_enabled`) |
| **What it settles** | The year's **whole** Kontokorrent movement, never a hand-picked subset of Buchungsarten; the monthly Abschlag is recalibrated from supply billing alone |
...
| **MCP** | 13 tools at `/mcp`, each authorized as its REST twin is; an unmapped tool is refused (issuing a reversal is deliberately not a tool — `list_sepa_collections` only reads, the reversal is an operator decision) |
...
| `de.accounting.sepa.recall-requested` | a camt.055 asked the bank to stop a collection before it settles |
| `de.accounting.sepa.recall-resolved` | a camt.029 decided the case |
