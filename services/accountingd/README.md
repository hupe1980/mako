...
| **Auth** | OIDC/JWT **+ Cedar ABAC** on every endpoint, plus inbound webhook HMAC-SHA256. Reads split three ways — `read-account`, `read-banking`, `read-books` — and the `/mcp` surface is held to the same actions, one per tool. `tests/authorization_guard.rs` fails the build on a lost `Claims` extractor, a lost Cedar check, or a mutating MCP tool mapped to a read |
...
| **Verzugszinsen** | `GET/POST /api/v1/accounts/{malo_id}/interest-charges` — §288 BGB on the §247 Basiszinssatz + 5 pp (B2C) / + 9 pp (B2B). Booked to a **Zinsertrag** GL account under its own Buchungsart (§275 HGB reports them on their own line). The Basiszinssatz moves on 1 January and 1 July, so a Verzugszeitraum is split at every Stichtag it crosses and each segment charged at its own rate; an unseeded period is refused, never estimated |
