//! Guard: every `demos/` config file `accountingd` loads is one `accountingd` accepts.
//!
...
//! with `ACCOUNTINGD_*` environment variables, so a key the struct does not declare is
...
const DEMO_CONFIGS: &[&str] = &["demos/o2c/accountingd.toml"];
...
fn the_demo_configs_name_no_key_the_service_ignores() {
    for rel in DEMO_CONFIGS {
        ...
        if let Err(e) = toml::from_str::<accountingd::config::AccountingdConfig>(&src) {
            let msg = e.to_string();
            assert!(
                !msg.contains("unknown field"),
                "{rel} names a key `accountingd` does not have — fix the demo, or the config:\n{msg}"
            );
        }
    }
}
...
fn the_demo_configs_start() {
    for rel in DEMO_CONFIGS {
        ...
        let Ok(cfg) = toml::from_str::<accountingd::config::AccountingdConfig>(&src) else {
            continue;
        };
        if let Err(e) = cfg.check_auth_posture() {
            panic!(
                "{rel} parses but `accountingd` refuses to start with it. Set \
