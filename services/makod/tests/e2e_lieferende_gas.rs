...
UNB+UNOC:3+4012345000023:14+9900357000004:500+250115:0900+GAS-2025-002'\
UNH+MSG-002+UTILMD:D:11A:UN:G1.1'\
BGM+E03:::+00044004::+9'\
DTM+137:202501150900?+00:303'\
RFF+Z13:GAS-REF-002'\
...
async fn e2e_lieferende_gas_negative_aperak() {
    ...
    assert_eq!(
        aperak_outbox.len(),
        1,
        "negative SendAntwort must enqueue one UTILMD answer outbox entry"
    );
    let aperak = &aperak_outbox[0];
    assert_eq!(aperak.message_type.as_ref(), "UTILMD");
