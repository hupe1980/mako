...
| `GaBiGasModule` | GaBi Gas — 5 workflows: INVOIC 31007/31008/31010, MSCONS 13013 (Allokationsliste MMMA), ALOCAT, NOMINT/NOMRES, SSQNOT | INVOIC 31007/31008/31010, REMADV 33001, COMDIS 29001, ORDERS 17110, MSCONS 13013, DVGW 70001–70023 (ALOCAT) / 70030–70039 (NOMINT/NOMRES) / 70095–70096 (SSQNOT) |
...
| `e2e_dispatch_coverage_guard.rs` | **Every registered PID must reach a dispatch arm.** Dispatches the profile's own skeleton of each registered Anwendungsfall and fails on any silent `pid_not_in_*` drop. Send-only PIDs — where mako initiates and no receiver exists — are enumerated in `SEND_ONLY_PIDS`; the guard fails if one starts being handled, so the list only shrinks |
