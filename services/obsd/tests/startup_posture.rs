//! Guard: obsd refuses to start in a posture that admits every caller.
//!
...
fn config(extra: serde_json::Value) -> Config {
    let mut cfg = serde_json::json!({"http": {}, "database": {"url": "postgres://localhost/obsd"}, "identity": {"tenant": "9900357000004"}, "marktd": {"url": "http://localhost:8180", "api_key": "test"}});
...
fn startup_refuses_without_oidc() {
    ...
    for named in ["[oidc]", "read model", "Antwortfristen", "Gleichbehandlung"] {
        assert!(
            msg.contains(named),
