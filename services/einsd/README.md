...
The operator guide — every statutory rule below, with its Fundstelle — is
[einsd on the docs site](https://hupe1980.github.io/mako/docs/services/einsd/).
...
| **Cumulative counters** | Four running totals the settlement both reads and writes (§ 8 Abs. 1–4 KWKG, § 44b, § 51a); `settlement_period_accruals` holds each period's contribution, so a re-settle applies the difference. They are re-read under a `FOR UPDATE` lock on the plant row as the transaction's first statement — the one point serialising every settle of a plant |
| **§9 Steuerbarkeit** | § 9 Abs. 2 Satz 1 stages the duty by capacity **and** gates it on the Veräußerungsform — Nr. 2 joins its two lit. with „und". Both routes are recorded per plant. |
| **§ 52 Pflichtzahlungen** | Abs. 1 lists **thirteen** breaches (Nr. 1–12 with a Nr. 9a). Four are derived from the plant record (`sect52`), nine are recorded in `eeg_pflichtverstoesse`, and the engine prices all of them — Abs. 2 rate, Abs. 3 reduction, Abs. 4 tail, Abs. 5 cap. |
| **§ 52 register** | `POST/GET /api/v1/anlagen/{tr_id}/pflichtverstoesse` and `PUT …/{typ}/behoben`. A record never *creates* a breach einsd derives — only refines it — so a cure cannot silence what the plant record still shows |
| **§10b against a non-direct-marketer** | Charges nothing: § 10b Abs. 1 binds only a plant that direct-markets |
...
| **Plant types** | 18 `erzeugungsart` values: five SOLAR Bauformen, WIND_ONSHORE/OFFSHORE, BIOMASSE/BIOMASSE_HOLZ/BIOGAS/BIOMETHAN, KLAERGAS/GRUBENGAS/DEPONIEGAS, WASSERKRAFT, GEOTHERMIE, GEZEITEN, KWKG. There is no generic `SOLAR` — the §48 rate depends on the Bauform |
| **Settlement models** | 12, one token each (no aliases): VERGUETUNG, AUSFALLVERGUETUNG (§21 Abs. 1 Satz 1 Nr. 3), MIETERSTROM (§21 Abs. 3), GGV (§42b EnWG), DIREKTVERMARKTUNG (§20), AUSSCHREIBUNG (§22), SONSTIGE_DIREKTVERMARKTUNG (§21a), POST_EEG_SPOT, EIGENVERBRAUCH, KWKG_ZUSCHLAG (§7 KWKG), FLEXIBILITAET (§50b), FLEXIBILITAET_ZUSCHLAG (§50a) |
...
| **Marktwert series** | `marktwert_preise` holds both Anlage 1 series and **Nr. 2 decides** which a plant takes, never the operator. The lookup never substitutes the other series; a miss is `PriceMissing`. `PUT/GET /api/v1/marktwert/{year}/{art}/{erzeugungsart}` (`art` = `monat` \| `jahr`). |
| **Provisional Jahresmarktwert** | A Jahresmarktwert has no month and is published as a running ÜNB estimate before the binding figure. A settlement computed on one stamps `settlement_receipts.marktwert_vorlaeufig`; `GET /api/v1/marktwert/{year}/nachbewertung` lists the periods that still need the § 147 AO correction once the final figure lands |
...
| **Zusammenlegung** | `parent_tr_id` links merged plants. The endpoint evaluates **§ 24 Abs. 1 in full** — the four cumulative conditions of Satz 1 plus the Sätze 2–5 carve-outs — and refuses an unsupported merge with `422`, naming the rule that decided. Ownership is not a criterion („unabhängig von den Eigentumsverhältnissen") |
...
| **KWKG Förderdauer** | §8 measures a KWK plant in Vollbenutzungsstunden, not in years, and caps each calendar year separately (Abs. 4). So a KWK plant has **no** calendar Förderende: `foerderendedatum` is NULL and `kwk_foerderdauer_h` ends the Zuschlag. |
...
| **§ 51 Negativpreisregel** | Keyed on the **Inbetriebnahmedatum**, not the law year — the Solarspitzengesetz rewrote § 51 inside the EEG 2023 range. `NegativpreisRegime::fuer_inbetriebnahme` gives the run-length threshold and the exemption; Pilotwindenergieanlagen are exempt up to the EEG 2023 Fassung, and in scope under the Solarspitzengesetz §51 Abs. 2, which names only the 100 kW and 2 kW thresholds |
| **§ 51 auto-derivation** | A settle without explicit values fetches the plant's ¼h feed-in from edmd and overlays the spot store over the **Europe/Berlin** billing month. A § 40a Abs. 2 EnWG billability gate skips the reduction below 95 % coverage; a genuine zero is recorded as a zero, not as "unknown" |
| **§7 KWKG Zuschlagssatz** | Abs. 1 and Abs. 2 are different ladders, each priced per Leistungsanteil, so a rate is the blended Mischsatz over the bands the plant spans. `kwk_verwendung` has **no default** — unset, the settlement refuses rather than assume the dearer ladder. |
...
| KWKG_ZUSCHLAG | `kwh × Mischsatz / 100`, where §7 prices each Leistungsanteil separately (8 / 6 / 5 / 4,4 ct up to 2 MW, then 3,4 or 3,1 ct by Anlagenart) and §7 Abs. 2 applies a lower ladder where the KWK-Strom is not fed into a Netz der allgemeinen Versorgung |
...
`einsd` exposes a Streamable HTTP MCP server at `/mcp`. Sixteen tools are
read-only; three write — `trigger_settle`, `import_marktwert` and
...
# Optional auto-import of the ÜNB **Monats**marktwert series (Anlage 1 Nr. 3).
# `{year}`/`{month}` are substituted per billing period. The key is named for
# the table it predates. The Jahresmarktwert of Nr. 4 lands once a year and is
# imported by hand; without this key both series are.
# jahresmarktwert_url = "https://api.netztransparenz.de/eeg/marktwert/{year}/{month}"
# jahresmarktwert_import_interval_secs = 86400
