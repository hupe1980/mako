...
| **Cumulative counters** | Four running totals the settlement both reads and writes (§8 Abs. 1–4 KWKG, §44b, §51a). They are re-read under a `FOR UPDATE` lock on the plant row as the transaction's first statement — the one point serialising every settle of a plant — and `settlement_period_accruals` holds each period's contribution, so a re-settle applies the difference |
