...
| `GET` | `/api/v1/malo/{malo_id}/produkte` | The valid-time product assignment — `?as_of=` or `?from=&to=`; each slice carries the `jahresverbrauch_kwh` that selects a Preisstaffel |
