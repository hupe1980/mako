//! Guard: every `demos/` config file `outputd` loads is one `outputd` accepts.
//!
...
//! with `OUTPUTD_*` environment variables, so a key the struct does not declare is
...
const DEMO_CONFIGS: &[&str] = &["demos/o2c/outputd.toml"];
...
fn the_demo_configs_name_no_key_the_service_ignores() {
    for rel in DEMO_CONFIGS {
        ...
        if let Err(e) = toml::from_str::<outputd::config::OutputdConfig>(&src) {
            let msg = e.to_string();
            assert!(
                !msg.contains("unknown field"),
                "{rel} names a key `outputd` does not have — fix the demo, or the config:\n{msg}"
            );
        }
    }
}
