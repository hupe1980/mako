...
use clap::Parser;
use mako_mdm::repository::AppState;
...
use mdmd::{
    ...
    openapi::swagger_ui,
    pg::{
        PgContractRepository, PgCorrelationIndex, PgMaloRepository, PgMeloRepository,
        PgPartnerRepository, PgSubscriptionRepository,
    },
};
...
struct Cli {
    /// Path to the `mdmd.toml` configuration file.
    #[arg(short, long, default_value = "mdmd.toml", env = "MDMD_CONFIG")]
...
async fn main() -> anyhow::Result<()> {
    ...
    info!(config = %cli.config.display(), "mdmd: starting");
    ...
    // ── PostgreSQL ────────────────────────────────────────────────────────────
    info!("mdmd: connecting to PostgreSQL");
    ...
    let http = reqwest::Client::builder()
        .user_agent("mdmd/0.1 (+https://github.com/hupe1980/edi-energy-rs)")
        .timeout(Duration::from_secs(10))
        .build()
        .context("building HTTP client")?;

    let verifier = OidcVerifier::new(&cfg.oidc.issuer, &cfg.oidc.audience, &http)
        .await
        .context("OIDC discovery")?;

    // ── MaKod client ──────────────────────────────────────────────────────────
    let makod_client = Arc::new(mako_mdm::makod_client::MakodClient::new(
        &cfg.makod.base_url,
        makod_api_key,
    ));
    ...
    verifier
        .clone()
        .spawn_refresh_task(http.clone(), cfg.oidc.jwks_refresh_secs, shutdown.clone());
    ...
    tokio::spawn(async move {
        ...
        info!("mdmd: SIGINT received, shutting down");
