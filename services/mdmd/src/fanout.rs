//! Fan-out worker — delivers `MdmEvent`s to matching webhook subscribers.
//!
...
pub fn spawn<S>(
    mut rx: mpsc::UnboundedReceiver<MdmEvent>,
    sub_repo: S,
    http: reqwest::Client,
    config: FanoutConfig,
...
async fn collect_subscribers<S>(sub_repo: &S, event: &MdmEvent) -> Vec<Subscription>
where
    S: SubscriptionRepository,
{
    let role = event.mdmrole.as_deref().unwrap_or("");
    let event_type = event.ce_type.as_str();
