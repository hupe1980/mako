//! Guard: every `demos/` config file `billingd` loads is one `billingd` accepts.
//!
...
//! with `BILLINGD_*` environment variables, so a key the struct does not declare is
...
const DEMO_CONFIGS: &[&str] = &["demos/o2c/billingd.toml"];
...
fn the_demo_configs_name_no_key_the_service_ignores() {
    for rel in DEMO_CONFIGS {
        ...
        if let Err(e) = toml::from_str::<billingd::config::BillingdConfig>(&src) {
            let msg = e.to_string();
            assert!(
                !msg.contains("unknown field"),
                "{rel} names a key `billingd` does not have — fix the demo, or the config:\n{msg}"
            );
        }
    }
}
