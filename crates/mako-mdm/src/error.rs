...
//! Domain error type for `mako-mdm`.
//!
//! No axum dependency here — `services/mdmd` maps `MdmError` to HTTP responses
//! via its own `IntoResponse` impl.

/// All domain-level errors produced by `mako-mdm`.
...
pub enum MdmError {
    ...
    #[error("invalid GLN '{gln}': {reason}")]
    InvalidGln { gln: String, reason: String },
...
impl MdmError {
    ...
    ///
    /// `services/mdmd` converts this to `axum::http::StatusCode`.
    #[must_use]
