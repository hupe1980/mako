...
//! the **"WiM Gas"** process family. This module is the canonical home of the
//! workflow; PID routing is performed by [`WimGasModule`][crate::WimGasModule].
//!
...
//! - **APERAK Frist: 10 Werktage** (same as all WiM Gas processes)
...
pub const WORKFLOW_NAME: &str = "wim-gas-stornierung";
...
/// Events emitted by the WiM Gas Stornierung workflow.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[serde(tag = "type", content = "data")]
pub enum WimGasStornierungEvent {
...
impl EventPayload for WimGasStornierungEvent {
    fn event_type(&self) -> &'static str {
        match self {
            Self::StornierungReceived { .. } => "WimGasStornierungReceived",
            Self::ValidationPassed { .. } => "WimGasStornierungValidationPassed",
            Self::ValidationFailed { .. } => "WimGasStornierungValidationFailed",
            Self::AperakDispatched { .. } => "WimGasStornierungAperakDispatched",
            Self::DeadlineExpired { .. } => "WimGasStornierungDeadlineExpired",
        }
    }
}
...
pub struct WimGasStornierungData {
...
/// Current state of a WiM Gas Stornierung process stream.
///
/// # Lifecycle
///
/// ```text
...
impl CommandPayload for WimGasStornierungCommand {}
...
pub struct WimGasStornierungWorkflow;

impl Workflow for WimGasStornierungWorkflow {
    type State = WimGasStornierungState;
    type Event = WimGasStornierungEvent;
    type Command = WimGasStornierungCommand;
    ...
    fn on_deadline(
        ...
        match (deadline.label(), state) {
            ...
            ) => Some(WimGasStornierungCommand::TimeoutExpired {
    ...
    fn apply(state: Self::State, event: &Self::Event) -> Self::State {
        match event {
            WimGasStornierungEvent::StornierungReceived {
                pruefidentifikator,
                sender,
                receiver,
                vorgang_id,
                document_date,
                message_ref,
            } => WimGasStornierungState::Initiated(WimGasStornierungData {
                pruefidentifikator: *pruefidentifikator,
                sender: sender.clone(),
            ...
            WimGasStornierungEvent::ValidationPassed { .. } => {
                if let WimGasStornierungState::Initiated(data) = state {
                    WimGasStornierungState::ValidationPassed(data)
                } else {
                    state
                }
            }

            WimGasStornierungEvent::ValidationFailed { errors } => {
                WimGasStornierungState::Rejected {
                    reason: errors.join("; "),
                }
            }

            WimGasStornierungEvent::AperakDispatched { positive, reason } => match state {
                WimGasStornierungState::ValidationPassed(data) => {
                    if *positive {
                        WimGasStornierungState::Completed(data)
                    } else {
                        WimGasStornierungState::Rejected {
                            reason: reason
                                .clone()
            ...
            WimGasStornierungEvent::DeadlineExpired { label, .. } => match state {
                WimGasStornierungState::Completed(_) | WimGasStornierungState::Rejected { .. } => {
                    state
                }
                _ => WimGasStornierungState::Rejected {
    ...
    fn handle(
        ...
        match command {
            WimGasStornierungCommand::ReceiveUtilmd {
                pid,
                sender,
                receiver,
                vorgang_id,
                ...
                if !matches!(state, WimGasStornierungState::New) {
                ...
                if validation_passed {
                    events.push(WimGasStornierungEvent::ValidationPassed { message_ref });
                } else {
                    events.push(WimGasStornierungEvent::ValidationFailed {
            ...
            WimGasStornierungCommand::DispatchAperak { positive, reason } => {
                if !matches!(state, WimGasStornierungState::ValidationPassed(_)) {
                ...
                Ok(WorkflowOutput::events(vec![
                    WimGasStornierungEvent::AperakDispatched { positive, reason },
                ]))
            }

            WimGasStornierungCommand::TimeoutExpired { deadline_id, label } => match state {
                WimGasStornierungState::Completed(_) | WimGasStornierungState::Rejected { .. } => {
                    Ok(WorkflowOutput::events(vec![]))
                }
                _ => Ok(WorkflowOutput::events(vec![
                    WimGasStornierungEvent::DeadlineExpired { deadline_id, label },
                ])),
            },
        }
    }
...
mod tests {
    ...
    fn stornierung_cmd(p: u32, validation_passed: bool) -> WimGasStornierungCommand {
        WimGasStornierungCommand::ReceiveUtilmd {
            pid: pid(p),
    ...
    fn receive_valid_44022_transitions_to_validation_passed() {
        let state = WimGasStornierungState::New;
        let output =
            WimGasStornierungWorkflow::handle(&state, stornierung_cmd(44022, true)).unwrap();
        assert_eq!(
            output.events.len(),
            2,
        ...
        let state = output
            .events
            .iter()
            .fold(state, WimGasStornierungWorkflow::apply);
        assert!(matches!(state, WimGasStornierungState::ValidationPassed(_)));
    }
    ...
    fn receive_invalid_44022_transitions_to_rejected() {
        let state = WimGasStornierungState::New;
        let output =
            WimGasStornierungWorkflow::handle(&state, stornierung_cmd(44022, false)).unwrap();
        assert_eq!(
            output.events.len(),
            2,
        ...
        let state = output
            ...
            .iter()
            .fold(state, WimGasStornierungWorkflow::apply);
        assert!(matches!(state, WimGasStornierungState::Rejected { .. }));
    }
    ...
    fn dispatch_positive_aperak_completes_process() {
        ...
        let output = WimGasStornierungWorkflow::handle(
            &state,
            WimGasStornierungCommand::DispatchAperak {
                positive: true,
                reason: None,
            },
        )
        ...
        let state = output
            ...
            .fold(state, WimGasStornierungWorkflow::apply);
        assert!(
            matches!(state, WimGasStornierungState::Completed(_)),
    ...
    fn reject_unsupported_pid() {
        let state = WimGasStornierungState::New;
        let result = WimGasStornierungWorkflow::handle(&state, stornierung_cmd(44021, true));
        assert!(result.is_err(), "PID 44021 must be rejected");
    }

    #[test]
    fn deadline_is_noop_for_completed_process() {
        let data = WimGasStornierungData {
        ...
        let state = WimGasStornierungState::Completed(data);
        let output = WimGasStornierungWorkflow::handle(
            &state,
            WimGasStornierungCommand::TimeoutExpired {
                deadline_id: DeadlineId::new(),
