...
///
/// COMDIS is the EDI@Energy message used to reject or dispute a previously
/// received REMADV (Remittance Advice) or IFTSTA (Status Report).
///
/// **Note on Prüfidentifikator**: Unlike all other EDI@Energy messages, COMDIS
/// stores its Prüfidentifikator in a top-level `RFF+Z13` segment rather than
...
impl_edi_energy_message!(ComdisMessage);
