...
static DIRECTORY_VALIDATOR_ORDCHG_1_2: LazyLock<DirectoryValidator> = LazyLock::new(|| {
    DirectoryValidator::new(
        "EDI@Energy-ORDCHG-1.2",
        segment_lookup,
        is_code_valid,
        suggest_code,
        expected_components,
...
pub(crate) fn directory_validator() -> &'static DirectoryValidator {
    &DIRECTORY_VALIDATOR_ORDCHG_1_2
}
...
fn rule_group_sg1_rff_max_occurrences(
    ...
    if count > 9_999 {
        issues.push(
            ...
            .with_rule_id("MIG-ORDCHG-MIG-1.2-GROUP-SG1-RFF-CARD-MAX")
            .with_segment("RFF".to_owned()),
        );
    }
}
...
fn rule_group_sg3_nad_max_occurrences(
    ...
    if count > 99 {
        issues.push(
            ...
            .with_rule_id("MIG-ORDCHG-MIG-1.2-GROUP-SG3-NAD-CARD-MAX")
            .with_segment("NAD".to_owned()),
        );
    }
}
...
fn rule_group_sg1_rff_min_occurrences(
    ...
    if count < 1 {
        issues.push(
            ...
            .with_rule_id("MIG-ORDCHG-MIG-1.2-GROUP-SG1-RFF-CARD-MIN")
            .with_segment("RFF".to_owned()),
        );
    }
}
...
fn rule_group_sg3_nad_min_occurrences(
    ...
    if count < 1 {
        issues.push(
            ...
            .with_rule_id("MIG-ORDCHG-MIG-1.2-GROUP-SG3-NAD-CARD-MIN")
            .with_segment("NAD".to_owned()),
        );
    }
}
...
fn rule_segment_order(segments: &[edifact_rs::Segment<'_>], issues: &mut Vec<ValidationIssue>) {
    ...
    const EXPECTED_HEADER_ORDER: &[&str] = &["UNH", "BGM", "DTM", "RFF", "NAD"];
    ...
    const DETAIL_GROUP_TRIGGERS: &[&str] = &["RFF", "NAD"];
...
static AHB_ALL_PACK_ORDCHG_1_2: LazyLock<Arc<ProfileRulePack>> = LazyLock::new(|| {
    let pack = ProfileRulePack::new("ORDCHG-AHB-1.2-ALL")
        .for_message_type("ORDCHG")
        .for_release("1.2");
    let pack = pack
...
static RELEASE_ORDCHG_FV20261001: LazyLock<Release> = LazyLock::new(|| Release::new("1.2"));

pub(crate) struct OrdchgFv20261001Profile;

impl Profile for OrdchgFv20261001Profile {
    fn message_type(&self) -> MessageType {
        MessageType::Ordchg
    }
    fn release(&self) -> &Release {
        &RELEASE_ORDCHG_FV20261001
    }
    ...
    fn source_document(&self) -> Option<&'static str> {
        Some("ORDCHG MIG 1.2, Publikationsdatum 01.04.2026")
    }
...
pub(crate) static PROFILE: OrdchgFv20261001Profile = OrdchgFv20261001Profile;
