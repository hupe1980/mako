...
fn rule_segment_order(segments: &[edifact_rs::Segment<'_>], issues: &mut Vec<ValidationIssue>) {
    const EXPECTED_ORDER: &[&str] = &[
        "UNH", "BGM", "RFF", "DTM", "CUX", "NAD", "CTA", "COM", "DOC", "MOA", "AJT", "FTX", "UNT",
    ];
    ...
    for seg in segments {
        if let Some(pos) = EXPECTED_ORDER[cursor..].iter().position(|&t| t == seg.tag) {
            ...
            issues.push(
                ...
                .with_rule_id("MIG-COMDIS-MIG-1.0g-ORDER")
...
static RELEASE_COMDIS_FV20261001: LazyLock<Release> = LazyLock::new(|| Release::new("1.0g"));

pub(crate) struct ComdisFv20261001Profile;

impl Profile for ComdisFv20261001Profile {
    fn message_type(&self) -> MessageType {
        MessageType::Comdis
    }
    fn release(&self) -> &Release {
        &RELEASE_COMDIS_FV20261001
    }
    ...
    fn source_document(&self) -> Option<&'static str> {
        Some("COMDIS AHB 1.0g, Stand 01.10.2026")
    }
...
pub(crate) static PROFILE: ComdisFv20261001Profile = ComdisFv20261001Profile;
