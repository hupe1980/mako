...
fn expected_components(tag: &str, idx: usize) -> Option<u8> {
    match (tag, idx) {
        ("UNH", 0) | ("UNT", 0) | ("UNT", 1) | ("NAD", 0) | ("FTX", 0) => Some(1),
        _ => None,
    }
}
...
static DIRECTORY_VALIDATOR_APERAK_2_2: LazyLock<DirectoryValidator> = LazyLock::new(|| {
    DirectoryValidator::new(
        "EDI@Energy-APERAK-2.2",
        segment_lookup,
        is_code_valid,
        suggest_code,
        expected_components,
...
pub(crate) fn directory_validator() -> &'static DirectoryValidator {
    &DIRECTORY_VALIDATOR_APERAK_2_2
}
...
fn rule_group_sg3_nad_max_occurrences(
    ...
    if count > 2 {
        issues.push(
            ...
            .with_rule_id("MIG-APERAK-MIG-2.2-GROUP-SG3-NAD-CARD-MAX")
            .with_segment("NAD".to_owned()),
        );
    }
}
...
fn rule_group_sg4_erc_max_occurrences(
    ...
    if count > 99_999 {
        issues.push(
            ...
            .with_rule_id("MIG-APERAK-MIG-2.2-GROUP-SG4-ERC-CARD-MAX")
...
fn rule_group_sg2_rff_min_occurrences(
    ...
    if count < 1 {
        issues.push(
            ...
            .with_rule_id("MIG-APERAK-MIG-2.2-GROUP-SG2-RFF-CARD-MIN")
            .with_segment("RFF".to_owned()),
        );
    }
}
...
fn rule_group_sg3_nad_min_occurrences(
    ...
    if count < 1 {
        issues.push(
            ...
            .with_rule_id("MIG-APERAK-MIG-2.2-GROUP-SG3-NAD-CARD-MIN")
            .with_segment("NAD".to_owned()),
        );
    }
}
...
static DTM_FORMATS: &[(&str, &[&str])] = &[("137", &["303"]), ("171", &["303"])];
...
fn rule_segment_order(segments: &[edifact_rs::Segment<'_>], issues: &mut Vec<ValidationIssue>) {
    ...
    check_order(&tree, segments, "MIG-APERAK-MIG-2.2-ORDER", issues);
}
...
static AHB_ALL_PACK_APERAK_2_2: LazyLock<Arc<ProfileRulePack>> = LazyLock::new(|| {
    let pack = ProfileRulePack::new("APERAK-AHB-2.2-ALL")
        .for_message_type("APERAK")
        .for_release("2.2");
    let pack = pack
...
static RELEASE_APERAK_FV20261001: LazyLock<Release> = LazyLock::new(|| Release::new("2.2"));

pub(crate) struct AperakFv20261001Profile;

impl Profile for AperakFv20261001Profile {
    fn message_type(&self) -> MessageType {
        MessageType::Aperak
    }
    fn release(&self) -> &Release {
        &RELEASE_APERAK_FV20261001
    }
    ...
    fn source_document(&self) -> Option<&'static str> {
        Some("APERAK MIG 2.2, Publikationsdatum 01.04.2026")
    }
...
pub(crate) static PROFILE: AperakFv20261001Profile = AperakFv20261001Profile;
