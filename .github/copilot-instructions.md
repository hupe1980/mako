...
services/agentd/          Multi-agent LLM orchestration daemon — **28 declarative manifests** in `agents/*.yaml` run by **agentplane** (journal-first durable runtime; `Runtime::builder` + `Agent::new(&manifest)`); manifests embedded via `include_str!` with a parse guard; **`src/builtin/mod.rs` is the subscription table ONLY** (name + specialty + trigger_patterns — the prompt lives in the manifest, never in Rust); `plane::Router` maps CloudEvent type → capability, `plane::Plane` runs one journaled run per subscribing specialist (**no race/first-wins mode**); `Activation` honours `[bundled_agents] enable_all`/`enable` (an unknown name is a startup failure); **no per-agent config overrides** (model/prompt/tools/ceilings are manifest-owned, digest-covered); redb journal at `journal_path` = §147 AO record, sealed by the key ring (GDPR Art. 17 erasure by case); mutating tools carry `requires_approval` + `spec.oversight` obligation with `on_expiry: deny`; `protected_fields` bind `/malo_id`+`/pid` to trusted sources; privileged/quarantined model pair (`Tainted<T>` labels); **no DLQ, no RAG endpoints** (durability replaces retries; `de.agent.session.dlq.exhausted` removed); anthropic/openai/bedrock drivers from agentplane; **A2A Agent Cards** at `/.well-known/agents/{name}`; `GET /api/v1/agents`, `GET /api/v1/agents/catalog`, `GET /api/v1/sessions`; CloudEvent trigger + `POST /api/v1/run`; **inbound HMAC** (`X-Mako-Signature: sha256=...`); **OIDC** on `POST /api/v1/run`; `max_sessions` semaphore (429); `session_timeout_secs` bounds the whole fan-out; **`SecretString`** for all secrets; emits de.agent.decision.made (outcome: completed/failed/suspended/exhausted/quarantined/replanning/cancelled; session_id = journal run id); port :9580
