...
services/mabis-syncd/      MaBiS Summenzeitreihe synchronisation daemon (ÜNB/NB role) — aggregates per-MaLo quarter-hourly Lastgang from edmd via `mako-mabis::SummenzeitreiheBuilder` (typed `metering::MeterInterval`, ¼-h slot length enforced); files one Summenzeitreihe per Bilanzierungsgebiet with the BIKO as MSCONS PID 13003 via the makod command `mabis.summenzeitreihe.uebermitteln`; ascending version per (Bilanzierungsgebiet, Bilanzierungsmonat) and BIKO-assigned Datenstatus per BK6-24-174 Anlage 3 §3.8.2/§3.8.3; submits on the `erstaufschlag_werktag` (default 10. Werktag) at 05:00 UTC per §3.10; `submission_runs` + `submission_malo_log` + `pruefmitteilung` PostgreSQL tables; inbound `POST /api/v1/datenstatus` (IFTSTA 21003/21004) + `POST /api/v1/pruefmitteilung` (21000/21001) + `GET /api/v1/korrekturbedarf`; OIDC + Cedar (`read-mabis-run`, `trigger-mabis-run` NB/ÜNB only); retry with attempt_count < 3 guard; MaLo discovery via edmd `/api/v1/billing-periods`, Bilanzierungsgebiet and MaBiS-Zählpunkt (`GET /api/v1/bilanzierungsgebiet/{eic}/mabis-zp`) via marktd — a territory with no assignment **refuses** its submission rather than substituting the EIC; `submission_target` selects `biko-bilateral` (implemented) or `mabis-hub` (BK6-24-210, refuses until the Beschluss lands); port :8880
services/agentd/          Multi-agent LLM orchestration daemon — **28 declarative manifests** in `agents/*.yaml` run by **agentplane 0.9** (journal-first durable runtime); manifests embedded via `include_str!` with a parse guard; **`src/builtin/mod.rs` is the subscription table ONLY** (name + specialty + trigger_patterns — the prompt lives in the manifest, never in Rust); `plane::Router` maps CloudEvent type → capability, `plane::Plane` runs one journaled run per subscribing specialist (**no race/first-wins mode**); **`plane::label` is the trust boundary** — a payload field is promoted to `Tainted::trusted` ONLY if re-validated against its identifier format (MaLo 11 digits, MP-ID 13, PID 5, MeLo 33, EIC 16, UUID, ISO date); everything else is `from_source(cloudevent:<type>)`; NEVER use `Runtime::run` (it labels input trusted) — always `run_tainted`; **two execution shapes**: 27 `tool-calling` (whole payload, per-field labels, NO quarantined model — 0.9 refuses one nothing selects) + `gabi-gas-agent` `planned` (routing envelope of re-validated ids only; plan compiled before untrusted data is read; counterparty text read by the **quarantined** model in a `parse` step); **`plane::tools`** connects one `McpClient` per `[mcp_servers]` entry over rmcp streamable-HTTP, registered via `RuntimeBuilder::tool_server` (a granted server missing from config = startup failure); `try_build` not `build` (build panics); `.tenant(cfg.tenant)` scopes erasure keys; `Activation` honours `[bundled_agents] enable_all`/`enable` (unknown name = startup failure); **no per-agent config overrides** (manifest-owned, digest-covered); redb journal at `journal_path` = §147 AO record, sealed by the key ring; mutating tools carry `requires_approval` + `spec.oversight` obligation with `on_expiry: deny`; `protected_fields` bind `/malo_id`+`/pid` to trusted sources; **no DLQ, no RAG endpoints** (durability replaces retries); **A2A Agent Cards** at `/.well-known/agents/{name}`; `GET /api/v1/agents`, `/agents/catalog`, `/sessions`; CloudEvent trigger + `POST /api/v1/run`; **inbound HMAC**; **OIDC** on `/api/v1/run`; `max_sessions` semaphore (429); `session_timeout_secs` bounds the whole fan-out; **`SecretString`** for all secrets; emits de.agent.decision.made (outcome: completed/failed/suspended/exhausted/quarantined/replanning/cancelled/not-admitted; session_id = journal run id); port :9580
