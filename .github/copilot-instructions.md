...
crates/mako-nne/          Role-neutral NNE/KA/MMM/MSB invoice calculation library — calculate_nne_invoice (PID 31001/31005/31006/31011 via PID override), calculate_mmm_invoice (PID 31002), calculate_msb_invoice (PID 31009); §14a Modul 2 HT/NT ToU split auto-generates 2 Arbeit positions when arbeitsmenge_ht_kwh+nt_kwh supplied; EuroAmount precision (i64 × 10⁻⁵ EUR); self-validates via invoic-checker; zero I/O; used by netzbilanzd (NB) and invoicd (LF selbstausstellen)
crates/eeg-billing/       Pure EEG/KWKG feed-in settlement library — calculate_settlement() for all 9 models (§§20–50a/b EEG + §7 KWKG 2023); EEG-version-aware rules via `EegGesetz` enum (Eeg2000–Eeg2023+Kwkg); §51 Negativpreisregel with Bestandsschutz (§66 EEG 2017 boundary 2016-01-01; EEG 2017: 6h/Wind<3MW/other<500kW; EEG 2021: 4h/<500kW; EEG 2023: any/<100kW); §52 Pflichtzahlungen via `SanktionsTyp` (€10/kW or €2/kW always for Nr.9a+Nr.10 per §52 Abs.3 Nr.2) + `SanktionAlt` (3-tier old-regime: VerguetungAufNull/AufMarktwert/Reduziert20Prozent); §53 Vergütungsabzug helper (0.4ct solar/wind, 0.2ct biomasse/gas); `foerderendedatum_eeg()` = Dec 31 of year+20 (§25 Abs.1 Satz 2); `foerderendedatum_eeg_ausschreibung()` = exact 20y; `ErzeugungsArt` enum (20 variants) for technology dispatch; `sect53_deduction()` helper; 190+ tests; zero float money; no I/O; used by einsd
...
services/einsd/           Einspeiser Registry + EEG/KWKG Settlement daemon — delegates to crates/eeg-billing; eeg_anlagen register (EegGesetz-typed, 20 ErzeugungsArt variants); 9 settlement models (VERGUETUNG, MIETERSTROM, DIREKTVERMARKTUNG, AUSSCHREIBUNG, POST_EEG_SPOT, EIGENVERBRAUCH, KWKG_ZUSCHLAG, FLEXIBILITAET, FLEXIBILITAET_ZUSCHLAG); MaStR lifecycle: `mastr_registriert` column + `angemeldet`→`aktiv` status + `POST /api/v1/anlagen/{tr_id}/mastr-registrierung` endpoint + `bank_iban`/`zahlungsempfaenger` for SEPA CT; `foerderendedatum` = Dec 31 year+20 for statutory, exact 20y for Ausschreibung; §51 version-aware rules via EegGesetz passed to eeg-billing; §52 SanktionAlt (old EEG) + SanktionsTyp Pflichtzahlung (EEG 2023); Repowering §22 + Zusammenlegung §24; KWKG Förderdauer; Managementprämie §20 Abs. 3; monthly auto-settle + batch settle; edmd auto-fetch; MCP /mcp (9 tools, 5 prompts); eeg-agent in agentd; port :9180
