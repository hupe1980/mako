...
| `eeg-billing` | Pure EEG/KWKG feed-in settlement library — `calculate_settlement` for all 9 models (\u00a7\u00a720\u201350b EEG 2023 + \u00a77 KWKG 2023); billing positions (`Vec<SettlePosition>`); \u00a725 MaStR sanctions; \u00a727 negative-price rule; `billing::LineItem` bridge; zero float money; no I/O |
| `energy-billing` | Pure multi-product retail energy billing — `calculate_strom/gas/waerme/solar/eeg/einspeisung/hems/emobility/energiedienstleistung/dynamic_strom`; all 12 product categories; §14a WAERMEPUMPE/WALLBOX Steuerungsrabatt Modul 1/3; §41a EPEX dynamic with optional price floor (`dynamic_epex_floor_ct_kwh`); §51 EEG Negativpreisregel (`kwh_during_negative_epex` contractual feature); regulatory rates (§3 StromStG, §2 EnergieStG, BEHG CO₂); BO4E `Rechnung` JSON; 44 tests; zero I/O, zero async |
...
| `einsd` | Einspeiser Registry + EEG/KWKG Settlement (NB/LF role) — plant register; **9 settlement models** (Vergütung, Mieterstrom §38a, Direktvermarktung Gleitende Marktprämie, Ausschreibung, Post-EEG Spot, Eigenverbrauch, KWKG-Zuschlag §7 KWKG 2023, Flexibilitätsprämie §50 EEG, Flexibilitätszuschlag §50b EEG); Repowering §22 EEG; Zusammenlegung §24; KWKG Förderdauer; built-in EEG 2000–2023 + KWKG 2023 rate table; CloudEvents `de.eeg.verguetung.berechnet` + `de.eeg.marktpraemie.berechnet` + `de.eeg.anlage.foerderung_auslaufend`; `:9180` |
...
| [einsd Operator Guide](./docs/einsd.md) | EEG/KWKG Settlement: 9 settlement models, Repowering §22, KWKG Förderdauer, CloudEvents |
